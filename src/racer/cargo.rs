...
macro_rules! otry2 {
    ($e:expr) => (match $e { Ok(e) => e, Err(e) => { error!("ERROR!: {:?} {} {}", e, file!(),
                                                            line!()); return None } })
}
...
fn get_repository_name_from_source(source: &str) -> Option<&str> {
    debug!(
        "get_repository_name_from_source - Finding repository name from {:?}",
        source
    );
    match source.rfind("/") {
        Some(idx) => {
            let (_, mut repository_name) = source.split_at(idx + 1);
            let mut idx = repository_name
                .find("?")
                .unwrap_or_else(|| repository_name.find("#").unwrap_or(0));
            if idx == 0 {
                return None;
            }
...
fn find_src_via_lockfile(kratename: &str, cargofile: &Path) -> Option<PathBuf> {
    trace!(
        "find_src_via_lockfile searching for {} in {:?}",
        kratename,
        cargofile
    );
                ...
                let package_source = match package_source.split('+').nth(0) {
                    ...
                    Some("git") => {
                        ...
                        if repository_name.is_some() {
                            d = unwrap_or_continue!(
                                find_git_src_dir(d, repository_name.unwrap(), &sha1, branch)
                            );
                        } else {
                            d = unwrap_or_continue!(
                                find_git_src_dir(d, package_name, &sha1, branch)
                            );
                        }
...
fn find_src_via_tomlfile(kratename: &str, cargofile: &Path) -> Option<PathBuf> {
    ...
    let local_packages_dev =
        get_local_packages(&table, cargofile, "dev-dependencies").unwrap_or_default();
    ...
    debug!(
        "find_src_via_tomlfile found local packages: {:?}",
        local_packages
    );
    debug!(
        "find_src_via_tomlfile found local packages dev: {:?}",
        local_packages_dev
    );
...
fn get_local_packages(
    ...
    debug!(
        "get_local_packages found table {:?};\
         getting packages for section '{}'",
        table,
        section_name
    );
    ...
    for (package_name, value) in t.iter() {
        ...
        let package_source = match *value {
            toml::Value::Table(ref t) => if let Some(relative_path) = getstr(t, "path") {
            ...
            _ => {
                trace!(
                    "get_local_packages couldn't find package_source for {}",
                    package_name
                );
                continue;
            }
    ...
    P: Into<PathBuf>,
{
    let path = path.into();
    trace!(
        "get_crate_file_from_overrides; crate_name={:?}, path={:?}",
        crate_name,
        path
    );
        ...
        for override_path in get_override_paths(cargo_config.as_path()).into_iter() {
            ...
            if path.is_relative() {
                ...
                let mut tmp = cargo_config
                    .parent()
                    .expect("config in path")
                    .parent()
                    .expect(".cargo in path")
                    .to_path_buf();
...
mod tests {
    ...
    fn get_crate_file_from_overrides() {
        ...
        assert_eq!(
            actual.canonicalize().expect("canonicalize path"),
