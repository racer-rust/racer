...
use util::{char_at, find_ident_end, is_ident_char, symbol_matches, txt_matches,
           StackLinkedListNode};
use nameres::{get_crate_file, get_module_file, resolve_path};
...
) -> iter::Chain<MChain<MChain<MChain<MChain<MChain<MIter>>>>>, vec::IntoIter<Match>> {
    let it = match_extern_crate(
        &src,
        blobstart,
        blobend,
        searchstr,
...
fn find_keyword(
    ...
    if src[start..].starts_with(search) {
        match search_type {
            ...
            ExactMatch => if src.len() > start + search.len() &&
...
pub fn match_for(
    ...
    for (start, end) in coords {
        ...
        if symbol_matches(search_type, searchstr, s) {
            debug!(
                "match_for point is {}, found ident {}",
                blobstart + start,
                s
            );
            out.push(Match {
...
pub fn match_mod(
    ...
    if let Some(start) = find_keyword(blob, "mod", searchstr, search_type, local) {
        ...
        if blob.find('{').is_some() {
            ...
            if let Some(modpath) =
                scopes::get_module_file_from_path(msrc, blobstart, filepath.parent().unwrap())
            {
...
pub fn match_use(
    ...
    if blob.contains('*') {
        ...
        for path in use_item.paths.into_iter() {
            ...
            if symbol_matches(search_type, searchstr, &path.ident) {
                // i.e. 'use foo::bar as searchstr'
            ...
            } else if ident == "" || ident == "self" {
                // i.e. no 'as'. e.g. 'use foo::{bar, baz}'
                ...
                if len == 1 && path.path.segments[0].name == searchstr {
                    ...
                    if path.path.segments.len() > 1 {
                        if symbol_matches(
                            search_type,
                            searchstr,
                            &path.path.segments.last().unwrap().name,
                        ) {
...
pub fn match_fn(
    ...
    let blob = &msrc[blobstart..blobend];
    let keyword = if is_const_fn(msrc, blobstart, blobend) {
        "const fn"
    } else {
        "fn"
    };
