...
pub fn get_tuplestruct_field_type(
    ...
    let structsrc = if let core::MatchType::EnumVariant = structmatch.mtype {
        ...
        let to = src[structmatch.point..]
            .find('(')
            .map(|n| {
                scopes::find_closing_paren(&src, structmatch.point + n + 1)
            })
