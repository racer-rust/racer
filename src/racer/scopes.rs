...
pub fn find_closing_paren(src: &str, pos: Point) -> Point {
    find_close(src.as_bytes()[pos..].iter(), b'(', b')', 0).map_or(src.len(), |count| pos + count)
}

pub fn find_closure_scope_start(
    src: Src,
    point: Point,
    parentheses_open_pos: Point,
) -> Option<Point> {
...
pub fn scope_start(src: Src, point: Point) -> Point {
    ...
    if curly_parent_open_pos > 0 && masked_src[..curly_parent_open_pos].ends_with("::{") {
        trace!(
            "scope_start landed in a use statement for {}; broadening search",
            point
        );
        curly_parent_open_pos = find_close(
            mask_comments(src.to(curly_parent_open_pos - 1))
                .as_bytes()
                .iter()
                .rev(),
            b'}',
...
pub fn find_let_start(msrc: Src, point: Point) -> Option<Point> {
    ...
    for step in 1..6 {
        let_start = msrc.from(scopestart)
            .iter_stmts()
            .find(|&(_, end)| scopestart + end > point);
        ...
        if start < point && end > point {
            ...
            if blob.starts_with("pub mod ") || blob.starts_with("mod ") {
                ...
                ast::parse_mod(p).name.map(|name| {
                    ...
                    get_local_module_path_(blob.from(newstart), point - start - newstart, out);
                });
            }
        }
    }
...
pub fn find_impl_start(msrc: Src, point: Point, scopestart: Point) -> Option<Point> {
        ...
        .find(|&(_, end)| end > len)
    {
        Some((start, _)) => {
            ...
            if blob.starts_with("impl") || blob.starts_with("trait") ||
...
fn finds_subnested_module() {
    ...
    let point = src.coords_to_point(&Coordinate { line: 3, column: 8 })
        .unwrap();
    let v = get_local_module_path(src.as_src(), point);
    assert_eq!("foo", &v[0][..]);
}
...
pub fn get_start_of_pattern(src: &str, point: Point) -> Point {
    ...
    for &b in src.as_bytes()[..point].iter().rev() {
        match b {
            ...
            _ => if levels == 0 && !util::is_pattern_char(char_at(src, i)) {
                return i + 1;
            },
        }
        i -= 1;
...
fn get_start_of_pattern_handles_variant2() {
    assert_eq!(
        4,
        get_start_of_pattern("bla, ast::PatTup(ref tuple_elements) => {", 36)
    );
}
...
fn expand_search_expr_handles_inline_closures() {
    assert_eq!(
        (0, 29),
        expand_search_expr("yeah::blah.foo(|x:foo|{}).bar", 27)
    )
}
#[test]
fn expand_search_expr_handles_a_function_arg() {
    assert_eq!(
        (5, 25),
        expand_search_expr("myfn(foo::new().baz().com)", 23)
    )
}
...
",
    ...
    let commentoffset = src.coords_to_point(&Coordinate {
        line: 3,
        column: 23,
    }).unwrap();
