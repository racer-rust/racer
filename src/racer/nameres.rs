...
use core::MatchType::{Builtin, Enum, FnArg, Function, Impl, MatchArm, Module, Struct, StructField,
                      Trait, TraitImpl};
use core::Namespace;

use util::{self, closure_valid_arg_scope, find_ident_end, symbol_matches, txt_matches};
                            ...
                            if m.matchstr == "Deref" {
                                ...
                                if let Some(pos) = impl_block.find('=') {
                                    ...
                                    if let Some(pos) = blob[deref_type_start..].find(';') {
                                        ...
                                        let deref_type =
                                            blob[deref_type_start..deref_type_end].trim();
                        ...
                        (name_path.segments.last(), trait_path.segments.last())
                    {
                        for gen_arg in generics.generic_args {
                                ...
                                gen_arg.bounds[0] == searchstr
                            {
                                ...
                                let self_path =
                                    core::Path::from_vec(false, vec![&contextm.matchstr]);
    ...
    if let Some(stmtstart) = scopes::find_stmt_start(msrc, scopestart) {
        ...
        // 'if let' can be an expression, so might not be at the start of the stmt
            ...
            if txt_matches(search_type, searchstr, &src) {
                let mut out = matchers::match_if_let(
                    &src,
                    0,
                    src.len(),
                    searchstr,
            ...
            if txt_matches(search_type, searchstr, &src) {
                let mut out = matchers::match_while_let(
                    &src,
                    0,
                    src.len(),
                    searchstr,
            ...
            if txt_matches(search_type, searchstr, &msrc[..scopestart]) {
                let mut out =
                    matchers::match_for(&src, 0, src.len(), searchstr, filepath, search_type, true);
                for m in &mut out {
                    m.point += stmtstart;
                }
                return out.into_iter();
            ...
            let arm = match masked_matchstmt[..point-matchstart].rfind("=>") {
                ...
                Some(arm) => {
                    ...
                    if let Some(next_arm) = masked_matchstmt[arm+2..].find("=>") {
                        let enum_start = scopes::get_start_of_pattern(&masked_matchstmt,
                                                                      arm+next_arm+1);
            ...
            for (start, end) in ast::parse_pat_idents(fauxmatchstmt) {
                let (start, end) = (
                    lhs_start + start - faux_prefix_size,
            ...
            search_closure_args(searchstr, preblock, stmtstart, filepath, search_type)
        {
...
pub fn search_scope(
    ...
    for (blobstart, blobend) in delayed_single_imports
        .into_iter()
        .chain(delayed_glob_imports)
    {
        // There's a good chance of a match. Run the matchers
        for m in run_matchers_on_blob(
            src,
            start + blobstart,
            start + blobend,
            searchstr,
    ...
    if let Some(vec) = search_closure_args(searchstr, &scopesrc[0..], start, filepath, search_type)
    {
        for mat in vec {
            out.push(mat)
        }
...
fn run_matchers_on_blob(
    ...
    let mut out = Vec::new();
    match namespace {
        Namespace::Type => for m in matchers::match_types(
            src,
            start,
            end,
            searchstr,
        ...
        Namespace::Value => for m in
            matchers::match_values(src, start, end, searchstr, filepath, search_type, local)
        {
            out.push(m);
            if let ExactMatch = search_type {
                return out;
...
pub fn resolve_name(
    ...
    let is_exact_match = match search_type {
        ExactMatch => true,
        StartsWith => false,
    };
        ...
        context.map(|m| {
            match m.mtype {
                ...
                Enum => {
                    ...
                    debug!(
                        "searching an enum '{}' (whole path: {:?}) searchtype: {:?}",
                        m.matchstr,
                        path,
                        search_type
                    );
                    ...
                    scopesrc.iter_stmts().nth(0).map(|(blobstart, blobend)| {
                        for m in matchers::match_enum_variants(
                            &filesrc,
                    ...
                    // TODO remove code duplication with the struct branch below.
            ...
            match m.mtype {
                ...
                Struct => {
                    ...
                    for m in search_for_impls(
                        ...
                        // deal with started with "{", so that "foo::{bar" will be same
                        // as "foo::bar"
                        let searchstr = match path[path.len() - 1].chars().next() {
                            Some('{') => &path[path.len() - 1][1..],
                            _ => path[path.len() - 1],
...
pub fn search_for_field_or_method(
    ...
    match m.mtype {
        ...
        Builtin => for m in search_for_impl_methods(
            &m,
            searchstr,
            m.point,
            &m.filepath,
